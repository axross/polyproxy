...
Apply this reference when writing the Markdown body of a `SKILL.md` or any reference file under the host project's skill root.
...
- MUST make every substantive section demonstrate its topic before listing requirements, regardless of heading depth.
...
- MUST NOT use hedging verbs such as `might`, `could`, or `try to` inside normative bullets.
...
- MUST preserve the description-then-guidelines pattern after splitting a section.
