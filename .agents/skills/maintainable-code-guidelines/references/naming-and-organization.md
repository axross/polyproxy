...
File names follow the source ownership layout: use kebab-case for source files when the exported concept is a phrase, keep common/core modules under `src/common/`, Obsidian modules under `src/obsidian/`, and tests next to the files they verify.

**Good Examples:**

> `src/obsidian/routes/obsidian.tsx`

> `src/obsidian/helpers/bridge-url.ts`

**Bad Examples:**

> `src/obsidian/routes/Obsidian.tsx`

> `src/obsidian/helpers/bridgeUrl.ts`
...
- MUST keep common Worker/runtime modules under `src/common/`.
- MUST keep Obsidian route registration under `src/obsidian/routes/`.
- MUST keep Obsidian server-rendered Hono JSX under `src/obsidian/views/`.
- SHOULD keep pure helpers flat under `src/common/helpers/` or `src/obsidian/helpers/` until a real subdomain grows large enough to justify a folder.
...
- SHOULD reserve default exports for single-entry modules such as `src/common/app.tsx` and `src/common/worker.tsx`.
