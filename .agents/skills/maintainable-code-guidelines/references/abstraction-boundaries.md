...
```mermaid
flowchart LR
  Key["/ob/:key"] --> Route["src/obsidian/routes/obsidian.tsx"]
  Route --> Storage["src/obsidian/helpers/short-bridge-link.ts"]
  Storage --> Validate["src/obsidian/helpers/validation.ts"]
...
- SHOULD keep `src/obsidian/routes/obsidian.tsx` as orchestration glue over helpers and views, not the place where key generation, schema, or URI logic is reimplemented.
- MUST NOT duplicate `BridgePayload` validation in routes or views; use `validateBridgePayload()` or `validateBridgePayloadSafe()`.
...
- MUST use `buildShortBridgeUrl()` for public HTTPS proxy links.
