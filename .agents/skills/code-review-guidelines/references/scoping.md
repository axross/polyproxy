...
- MUST read `src/helpers/validation.ts`, `src/helpers/decode-link.ts`, and relevant tests for validation changes.
- MUST read `src/routes/obsidian.tsx`, affected `src/views/**`, and related helpers for route changes.
