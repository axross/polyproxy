...
describe("scrubSentryEvent", () => {
  it("redacts bridge route details from request fields", () => {
    const event = scrubSentryEvent({
      request: {
        url: "https://open.axross.dev/obsidian/eyJ2YXVsdCI6IlBlcnNvbmFsIn0",
    ...
    expect(event.request?.url).toBe("https://open.axross.dev/obsidian/[query]");
  ...
  it("redacts Obsidian URIs and sensitive breadcrumb data", () => {
    const event = scrubSentryEvent({
      breadcrumbs: [
        {
          ...
          data: {
            href: "https://open.axross.dev/obsidian/abc123",
            navigationType: "push",
          },
        },
      ],
      exception: {
        values: [
          {
            value:
              "Failed at /obsidian/abc123 for obsidian://open?vault=Personal",
          },
        ],
      },
      extra: {
      ...
      tags: {
        route: "/obsidian/abc123",
      },
      transaction: "GET /obsidian/abc123",
    });
    ...
    expect(event.exception?.values?.[0]?.value).toBe(
      "Failed at /obsidian/[query] for [Filtered]",
    );
    expect(event.extra?.path).toBe("[Filtered]");
    expect(event.extra?.operation).toBe("open");
    expect(event.tags?.route).toBe("/obsidian/[query]");
    expect(event.transaction).toBe("GET /obsidian/[query]");
  });
});
